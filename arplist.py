...
def getall(ip):
    ...
    # 输入命令
    arplist = net_connect.send_command('dis arp all | include 10GE')
    # 匹配每一行
    abc = u"(.+GE.+)"
    ...
    for i in range(len(result)):
        ...
        # vpn实例
        vpnlist = re.findall(r'\S*vRoute\S*',result[i])
        # 设备名
        decvicename = net_connect.send_command('display current-configuration | include sysname')
